...
from datasets.load_vpr_matches import load_vpr_matches
class SegHelper:
...
class changesim_eval(Dataset):
    ...
    def __init__(self, root,
        ...
        self.vpr_patchnetvlad = vpr_patchnetvlad
        self.vpr_netvlad = vpr_netvlad
        self.vpr_candidates = vpr_candidates
        if self.vpr_patchnetvlad or self.vpr_netvlad or self.vpr_candidates:
            self.paths = load_vpr_matches(dataset_name='changesim',query_root=root,seqname=seqname,
                                          vpr=(self.vpr_patchnetvlad,self.vpr_netvlad))
    ...
    def getitem_vpr(self,index):

        fn_t0s = self.paths['refs'][index]
        ...
        if os.path.isfile(fn_t0s[0]) == False:
            print('Error: File Not Found: ' + fn_t0s[0])
        ...
        img_t0s = [cv2.imread(fn_t0, 1) for fn_t0 in fn_t0s]
        img_t0s = [cv2.cvtColor(img_t0,cv2.COLOR_BGR2RGB) for img_t0 in img_t0s]
        img_t1 = cv2.imread(fn_t1, 1)
        img_t1 = cv2.cvtColor(img_t1,cv2.COLOR_BGR2RGB)

        h_r, w_r = self.img_size

        img_t0s_r = [cv2.resize(img_t0, (h_r, w_r)) for img_t0 in img_t0s]
        img_t1_r = cv2.resize(img_t1, (h_r, w_r))
        mask_r = cv2.resize(mask, (h_r, w_r),interpolation=cv2.INTER_NEAREST)
        img_t0s_r_ = [np.asarray(img_t0_r).astype('uint8') for img_t0_r in img_t0s_r]
        # img_t0_r_ = np.stack(img_t0s_r_,axis=0) # (20,3,h,w)
        ...
        if self.source_image_transform is not None:
            if isinstance(self.source_image_transform,transforms.Compose):
                img_t0s_r_ = [self.source_image_transform(img_t0_r_) for img_t0_r_ in img_t0s_r_]
            elif isinstance(self.source_image_transform,A.Compose):
                img_t0s_r_ = [self.source_image_transform(image=img_t0_r_)['image'] for img_t0_r_ in img_t0s_r_]
            else: raise ValueError
            img_t0_r_ = torch.stack(img_t0s_r_, dim=0)  # (20,3,h,w)
        if self.target_image_transform is not None:
            if isinstance(self.target_image_transform,transforms.Compose):
                img_t1_r_ = self.target_image_transform(img_t1_r_)
            elif isinstance(self.target_image_transform,A.Compose):
                img_t1_r_ = self.target_image_transform(image=img_t1_r_)['image']
            else: raise ValueError
    ...
    def getitem_normal(self,index):
        ...
        if type(img_t0) == type(None):
            print(f'{fn_t0} is not loaded!!')
        img_t0 = cv2.cvtColor(img_t0,cv2.COLOR_BGR2RGB)
        img_t1 = cv2.imread(fn_t1, 1)
        if type(img_t1) == type(None):
            print(f'{fn_t1} is not loaded!!')
        img_t1 = cv2.cvtColor(img_t1,cv2.COLOR_BGR2RGB)
    ...
    def __getitem__(self, index):
        if self.vpr_candidates:
            return self.getitem_vpr(index)
        else:
            return self.getitem_normal(index)

    def __len__(self):
        # if 'fire' in self.seqname:
        #     return len(self.paths['GT']) -40 -14

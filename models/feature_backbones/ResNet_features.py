...
class ResNetPyramid(nn.Module):
    def __init__(self, train=True, dense_cl=False):
        super().__init__()
        self.n_levels = 5
        self.model = models.resnet18(pretrained=True)
        
        if dense_cl:
            dense_cl_res = torch.load('./models/feature_backbones/densecl_r50_imagenet_200ep.pth')
            self.model.load_state_dict(dense_cl_res['state_dict'], strict=False)  # missing: fc.weight, fc.bias
            print("The weights of DenseCL are loaded!!")
    ...
    def forward(self, x, quarter_resolution_only=False, eigth_resolution=False):
        ...
        if quarter_resolution_only:
            ...
            x_2 = self.__dict__['_modules']['level_0'](x)
            outputs.append(x_2)
            x_4 = self.__dict__['_modules']['level_1'](x_2)
            outputs.append(x_4)
            x_8 = self.__dict__['_modules']['level_2'](x_4)
            outputs.append(x_8)
            x_16 = self.__dict__['_modules']['level_3'](x_8)
            outputs.append(x_16)
            x_32 = self.__dict__['_modules']['level_4'](x_16)
            outputs.append(x_32)
        else:
            # x = self.__dict__['_modules']['level_' + str(0)](x)
            # outputs.append(x)
            # outputs.append(x)  # [H/2, H/2]
            ...
            outputs.append(x)
            # it will contain [H/2, H/2, H/4, H/8, H/16, H/32, H/64]
        return outputs
